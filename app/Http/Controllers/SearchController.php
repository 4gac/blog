...
class SearchController extends Controller
{
	...
	public function countrySearch($id){
		...
		$renderData = DB::table('univerzita')
								->join('countries', 'countries.id', '=', 'univerzita.countries_id')
								->join('mesto', 'mesto.id', '=', 'univerzita.mesto_id')
								->select('univerzita.id', 'univerzita.nazov', 'univerzita.zmluva_od', 'univerzita.zmluva_do', 'mesto.nazov as nazovmesto', 'countries.name as nazovcountry', 'univerzita.kontaktna_osoba', 'countries.continent')
								->where('univerzita.countries_id', '=', $id)
								->orderBy('mesto.nazov', 'desc')
								->get();
		return view("search-results.show-country")->with('renderData',$renderData)->with('country',$country);
	}
